...
<template>
  <div class="flex flex-col min-h-screen">
    <div class="container mx-auto px-4 py-6 space-y-6 max-w-5xl flex-grow">
      <!-- Slider Section -->
      <div class="rounded-md mx-2 w-full overflow-hidden">
        <Slider class="w-full rounded-md mb-3" />
      </div>

      <!-- Date Filters & Search Section -->
      <div class="mx-2">
        <h2 class="text-sm font-semibold mb-1.5 text-gray-800">Filter Events</h2>
        <div class="flex flex-wrap items-center justify-between gap-2 bg-violet-50 p-4 rounded-md shadow-sm text-xs">
          <!-- Date Pickers -->
      ...
      <div class="mx-2">
        ...
        <!-- Events Grid -->
        <div v-else-if="eventStore.events.length" class="flex flex-col bg-violet-50 rounded-md shadow-sm p-4 text-xs">
          <EventCard v-for="event in eventStore.events" :key="event.id" :event="event" />
        </div>

        <!-- No Events Found -->
        <div v-else class="flex justify-center items-center h-64 text-gray-500 text-sm">
          No events available.
        </div>
