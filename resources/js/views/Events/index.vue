    ...
    <div class="flex flex-wrap items-center justify-between gap-2 bg-violet-50 p-3 rounded-md shadow-sm text-xs">
        ...
        <div class="flex flex-col gap-0.5">
          <Select v-model:modelValue="categoryFilter" @update:modelValue="searchEvents">
            ...
            <SelectContent>
              ...
              <SelectItem v-for="category in categoryStore.categories" :key="category.id" :value="String(category.id)">
                {{ category.name }}
              </SelectItem>
            </SelectContent>
          </Select>
      ...
      <div class="flex flex-col gap-0.5">
        <div class="relative w-[140px]">
          ...
          <Input v-model="searchQuery" placeholder="Search events..." class="pl-6 h-7 text-xs" @input="searchEvents" />
        </div>
      </div>
    </div>
  </div>
